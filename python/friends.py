...
class FriendListParser(HTMLParser):
    ...
    def handle_endtag(self,tag):
        if(tag=='ol'):
            ...
            print
            self.info=[]
            if(self.map.has_key(self.personid)==False):
                self.map[self.personid] = 1
                self.queue.append(self.personid)
...
class GetFriends():
    ...
    def FriendsOfSome(self,personid):
        ...
        html = response.read()
        self.name = self.GetName(html.decode('utf-8'),personid)
        self.file = open("d:/profiles/"+self.name+"."+personid+".txt","w")
    ...
    def Pop(self):
        personid = self.parser.queue.pop(0)
        #self.map.pop(personid)
        return personid

    def Insert(self,personid):
        if(self.parser.map.has_key(personid)):
            return False
        self.parser.queue.append(personid)
        self.parser.map[personid] = 1
        return True
